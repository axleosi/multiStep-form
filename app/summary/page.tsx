...
const Summary = () => {
  ...
  const addSumsMon = () => {
    const sum1 = planArrayMon.reduce((acc, plan) => acc + plan.latestAmount.amount, 0);
    const sum2 = billingMon.reduce((acc, plan) => acc + plan.latestAmount.amount, 0);
    return sum1 + sum2;
  };

  const addSumsYr = () => {
    const sum1 = planArrayYr.reduce((acc, plan) => acc + plan.latestAmount.amount, 0);
    const sum2 = billingYr.reduce((acc, plan) => acc + plan.latestAmount.amount, 0);
    return sum1 + sum2;
  };
        ...
        {!plan.isActive ? (
          <div className={styles.mapCon}>
            <div className={styles.miniMapCon}>
              {billingMon.map((billing, index) => (
                <div key={index} className={styles.plansCon}>
            ...
            <div className={styles.addonCon}>
              {planArrayMon.map((plan, index) => (
                <div key={index} className={styles.addonText}>
          ...
          <div className={styles.mapCon}>
            <div className={styles.miniMapCon}>
              {billingYr.map((billing, index) => (
                <div key={index} className={styles.plansCon}>
                  <div className={styles.planText}>
                    <p>{billing.latestPlan.plan}</p>
                    <Link href="/plan" className={styles.plansLink}>Change</Link>
                  </div>
                  <p>${billing.latestAmount.amount}/yr</p>
                </div>
              ))}
            </div>
            ...
            <div className={styles.addonCon}>
              {planArrayYr.map((plan, index) => (
                <div key={index} className={styles.addonText}>
                  <p>{plan.latestPlan.plan}</p>
                  <p>+${plan.latestAmount.amount}/yr</p>
                </div>
              ))}
            </div>
